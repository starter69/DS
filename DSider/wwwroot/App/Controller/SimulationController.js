...
jQuery(document).ready(function () {
    jQuery('#btnApproveSharedProject,.Carbon_MitigationLevel2,.Green_HydrogenModel,.Carbon_MitigationLevel1,.realEstateLevel,.GeoThermalLevel').hide();
    ...
    if (modelType == 'Green_Hydrogen') {
        ...
        jQuery('#ddIconTypes option[value=Green_HydrogenModel]').prop('selected', true);
    }
    else if (modelType == 'Carbon_Mitigation') {
        jQuery('.Carbon_MitigationLevel1').show().css('display', 'inline-block');
        jQuery('#ddIconTypes option[value=Carbon_MitigationLevel1]').prop('selected', true);
    }
    ...
    jQuery(document).on('click', '#duplicateComponent', function () {
        ...
        alertify.confirm("Are you sure to duplicate?", function (e) {
            if (e) {
                ...
                var newID = (editor.addNode(infoOfCurrentNode.name, inputCount, outputCount, parseFloat(infoOfCurrentNode.pos_x) + 20,
                    parseFloat(infoOfCurrentNode.pos_y) + 20, infoOfCurrentNode.class, infoOfCurrentNode.data, infoOfCurrentNode.html));
                //
                //Update Name
                var mProperties = {};
                ...
                if (mProperties.hasOwnProperty('Name')) {//if before added this property we should edit this propery
    ...
    jQuery(document).on('focusout', '.propertyItem', function () {
        ...
        if (mProperties.hasOwnProperty(property)) {//if before added this property we should edit this propery
            mProperties[property] = propertyValue;
        }
        else {//if this property didn't add before
            mProperties[property] = propertyValue;
        }
        editor.updateNodeDataFromId(mID, JSON.stringify(mProperties));
        //
        if (property == "Name") {
            jQuery('#node-' + mID).find('.drawflow_content_node').find('.componentTitle').text(propertyValue)
            editor.updateHtml(mID, jQuery('#node-' + mID).find('.drawflow_content_node').html())
        }
    });
    ...
    jQuery(document).on('click', function (e) {
        if (e.target.className != 'iDesign' && e.target.className != 'title-box' && e.target.className != 'componentTitle')
    ...
    jQuery(document).on('click', '.btnBrowseDataSource', function () {
        ...
        jQuery(dataSourcesInfo).each(function (Index, Val) {
            ...
            htmlCode += '<label for="group-' + Index + '"><span class="fa fa-angle-right"></span> ' + Val.name + '</label>';
            htmlCode += '<ul class="group-list">';
            jQuery(Val.columnsList).each(function (colIndex, colVal) {
                htmlCode += '<li class="DataSourcecolumn" dataSource="' + Val.name + '" coloumn="' + colVal + '"><a href="#">' + colVal + '</a></li>';
            });
            htmlCode += '</ul></li>';
    ...
    jQuery(document).on('click', '.DataSourcecolumn', function () {
        ...
        jQuery('.propertyItem[property="' + selectedProperyForBrowse + '"]').val(selectedDataSourceForCol + ' | ' + selectedColumn).trigger('focusout');
    ...
    jQuery(document).on('click', '#btnExportJSONFile', function () {
        jQuery('#fakeHrefForDownload').attr('href', '/Home/downloadExportJSON/' + subProjectID).attr('target', '_blank');
        ...
        alertify.confirm("Are you sure to delete?", function (e) {
            if (e) {
                if (typeDelete == 'userDefinedObjectIcons')
                    $.ajax({
                        url: '/api/WebAPI_UserDefinedObjects/deleteUserDefinedObject/' + selectedUserdefindeID,
                        type: "GET",
                        contentType: 'application/json',
                        success: function (response) {
                            getUserDefinedObjects();
    ...
    jQuery(document).on('click', '.deleteFromMacroList', function () {
        ...
        alertify.confirm("Are you sure to delete?", function (e) {
            if (e) {
                jQuery('#list').find('li[nodeID="' + mID + '"]').removeClass('show');
                jQuery('#node-' + mID).find('.checkASMacro').remove();
                setTimeout(function () {
    ...
    jQuery(document).on('contextmenu', '.userDefinedObjectIcons,.userMacroIcons', function (e) {
    ...
    jQuery(document).on('click', '.componentComment', function () {
        ...
        jQuery(filteredCommets).each(function (Index, Val) {
            ...
            if (Val.registerUser != lastUser) {
                if (chatStatus == '')
                    chatStatus = 'chat-left';
                else
                    chatStatus = '';
            }

            htmlCode += '<div class="chat ' + chatStatus + '"><div class="chat-avatar"><a class="avatar" data-toggle="tooltip" href="#" data-placement="right" title="' + Val.registerUser + '">';
            htmlCode += '<img src="/Images/avatar.png" alt="June Lane"></a></div>';
            htmlCode += '<div class="chat-body">';
            htmlCode += '<div class="chat-content"><div class="userChatInfo">' + Val.registerUser + '</div><p>' + Val.commentText + '</p><time class="chat-time" datetime="' + Val.registerDate + '">' + Val.registerDate + '</time>';
        ...
        alertify.confirm("Are you sure to delete?", function (e) {
            if (e) {
                ...
                if (mSelectedNodeForDelete.indexOf('input_') != -1) {
                    ...
                    if (mComponent.inputs.hasOwnProperty('input_1') || mComponent.inputs.hasOwnProperty('input_2')) {
                        for (var key in mComponent.inputs) {
                            jQuery(mComponent.inputs[key].connections).each(function (index, Val) {
                                if (key == mSelectedNodeForDelete) {
                                    editor.removeSingleConnection(parseInt(Val.node), parseInt(mID), Val.input, mSelectedNodeForDelete);
                                }
                            });
                        }
                    }
                ...
                if (mSelectedNodeForDelete.indexOf('output_') != -1) {
                    ...
                    if (mComponent.outputs.hasOwnProperty('output_1') || mComponent.outputs.hasOwnProperty('output_2')) {
                        for (var key in mComponent.outputs) {
                            jQuery(mComponent.outputs[key].connections).each(function (index, Val) {
                                if (key == mSelectedNodeForDelete) {
                                    editor.removeSingleConnection(parseInt(mID), parseInt(Val.node), mSelectedNodeForDelete, Val.output);
                                }
                            });
                        }
                    }
    ...
    jQuery(document).on('dblclick', '.drawflow-node', function () {
        ...
        jQuery('.RefineryLevel,.FleetLevel,.realEstateLevel,.Green_HydrogenModel,.GeoThermalLevel').hide();
        ...
        for (var key in mProperties) {
            if (key == 'Name') {
                ...
                if (jQuery('.detailsLevel').css('display') != 'none')//Level2
                    jQuery('#Level2Container').text(mProperties[key]).attr('currentNodeID', $mSelectedNodeFordblClick.attr('id')).attr('componentType', mComponent.name);
                else
                    jQuery('#detailsLevelContainer').text(mProperties[key]).attr('currentNodeID', $mSelectedNodeFordblClick.attr('id')).attr('componentType', mComponent.name);
            }
        }
        ...
        if (mCurrentComponentName.toLowerCase().indexOf('refinery') != -1) {
            jQuery('.Carbon_MitigationLevel1,.Carbon_MitigationLevel2,.GeoThermalLevel').hide();
            ...
            jQuery('.Carbon_MitigationLevel1,.Carbon_MitigationLevel2,.realEstateLevel,.GeoThermalLevel').hide();
            jQuery('.Green_HydrogenModel').show().css('display', 'inline-block');
        }
        else if (mCurrentComponentName.toLowerCase().indexOf('geothermal') != -1) {
            jQuery('.Carbon_MitigationLevel1,.Carbon_MitigationLevel2,.realEstateLevel,.Green_HydrogenModel').hide();
    ...
    jQuery(document).on('change', '#ddIconTypes', function () {
        ...
        jQuery('.Carbon_MitigationLevel1,.Carbon_MitigationLevel2,.realEstateLevel,.Green_HydrogenModel,.GeoThermalLevel,.FleetLevel,.RefineryLevel').hide();
        jQuery('.' + mSelectedItem).show().css('display', 'inline-block');
    });
});
...
function getComponentDetails(isBackPressed, gotoLevel) {
    ...
    if (levelNumber == '0') {
        ...
        jQuery('.Carbon_MitigationLevel1,.RefineryLevel,.realEstateLevel,.Green_HydrogenModel,.GeoThermalLevel').hide();
        //
        jQuery('.Carbon_MitigationLevel2,.RefineryLevel,.FleetLevel,.realEstateLevel,.Green_HydrogenModel').hide();
    ...
    if (mNodeName.toLowerCase().indexOf('refinery') != -1) {
        jQuery('.Carbon_MitigationLevel1,.Carbon_MitigationLevel2,.Green_HydrogenModel,.GeoThermalLevel').hide();
        jQuery('.RefineryLevel').show().css('display', 'inline-block');
    }
        ...
        jQuery('.Carbon_MitigationLevel1,.Carbon_MitigationLevel2,.realEstateLevel').hide();
        jQuery('.Green_HydrogenModel').show().css('display', 'inline-block');
    }
    ...
    $.ajax({
        url: '/api/WebAPI_Design/getComponentsDetailsModel/' + subProjectID + '/' + mID + '/' + mNodeName + '/' + levelNumber,
        type: "GET",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            ...
            if (typeof response == 'undefined')
                return;
            if (typeof response.exportJSON != 'undefined' && response.exportJSON != '' && response.exportJSON != null)
...
function saveComponentDetailsModel(isBackPressed, gotoLevel) {
    if (jQuery('.detailsLevel2').css('display') == 'none' && jQuery('.detailsLevel').css('display') == 'none') {//if was in root level
        return;
    }
...
function saveModel(statusType) {
    ...
    jQuery('.drawflow-node').each(function (Index, Val) {
        ...
        mInfo.push({
            ...
            'subProjectID': subProjectID,
            'priority': priority,
            'inputConnectionsCount': jQuery('#node-' + mComponent.id).find('.input').length.toString(),
            'outputConnectionsCount': jQuery('#node-' + mComponent.id).find('.output').length.toString()
        });
    });
...
function getexportData(levelNumber) {
    $.ajax({
        ...
        success: function (response) {
            ...
            if (typeof response.exportJSON != 'undefined' && response.exportJSON != '' && response.exportJSON != null)
                editor.import(JSON.parse(response.exportJSON));
            if (typeof response.workspaceZoomLevel != 'undefined' && response.workspaceZoomLevel != null)
                workspaceZoomLevel = response.workspaceZoomLevel;
            else
                workspaceZoomLevel = '1';
            //
...
function getConnectionsForModal() {
    ...
    for (var i = 0; i < inputCount; i++) {
        jQuery('#inputList').append('<li class="show" nodeID="input_' + (i + 1).toString() + '"><span>input - ' + (i + 1) + '<span><a  nodeID="input_' + (i + 1).toString() + '" href="#" class="inputItem deleteFromInputOutputNodes" style="float:right"><i class="far fa-trash-alt"></i></a><li>')
    }

    for (var i = 0; i < outputCount; i++) {
        jQuery('#outputList').append('<li class="show" nodeID="output_' + (i + 1).toString() + '"><span>output - ' + (i + 1) + '<span><a nodeID="output_' + (i + 1).toString() + '" href="#" class="outputItem deleteFromInputOutputNodes" style="float:right"><i class="far fa-trash-alt"></i></a><li>')
    }
}
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            jQuery(response).each(function (Index, Val) {
                jQuery('#' + Val.nodeID).append('<div class="componentComment" nodeID="' + Val.nodeID + '" compnentID="' + Val.id + '" title="Click to show comments"><i class="fas fa-comments"></i></div>')
            });
        },
        error: function (response) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.length > 0) {
                if (response[0].permission == 'Approve' && response[0].sharedProjectStatus != 'Approved')
                    jQuery('#btnApproveSharedProject').show();
                else if (response[0].permission == 'Approve' && response[0].sharedProjectStatus == 'Approved')
                    jQuery('#btnApproveSharedProject,#btnSave').hide();
                else if (response[0].permission == 'View')
                    jQuery('#btnSave').hide();
            }
        },
        error: function (response) {
...
function getUserMacros() {
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.length > 0) {
                jQuery(response).each(function (Index, Val) {
                    var htmlCode = '<div mID="' + Val.id + '" customName="Macro_' + Val.name + '" mID="' + Val.id + '" class="drag-drawflow userMacroIcons" draggable="true" ondragstart="drag(event)" data-node="' + Val.name + '">';
                    htmlCode += '<img src="../Images/Macro.png" title="' + Val.name + '" data-node="Macro_' + Val.name + '"/>';
                    //htmlCode += '<div mID="' + Val.id + '" draggable="false" class="deleteUserMacro"><a href="#">&#215;</a></div>';
                    htmlCode += '<div draggable="false" class="macroTitle">' + Val.name + '</div>';
                    htmlCode += '</div>';
...
function saveMacro() {
    ...
    var mInfo = [];
    jQuery('#list').find('.show').each(function (Index, Val) {
        var priority = '0';
...
function addToMacroList(id) {
    ...
    jQuery('#node-' + mComponent.id).append('<i class="far fa-check-circle checkASMacro"></i>');
    jQuery('#list').append('<li nodeID="' + mComponent.id + '"><span>' + componentName + '<span><a href="#" class="deleteFromMacroList" style="float:right"><i class="far fa-trash-alt"></i></a><li>')
    setTimeout(function () {
        jQuery('#list').find('li[nodeID="' + mComponent.id + '"]').addClass('show');
    }, 10);
}
...
function getUserDefinedObjects() {
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.length > 0) {
                jQuery(response).each(function (Index, Val) {
                    var htmlCode = '<div mID="' + Val.id + '" customName="userObject_' + Val.name + '" class="drag-drawflow userDefinedObjectIcons" draggable="true" ondragstart="drag(event)" data-node="turbine">';
                    htmlCode += '<img src="https://dsiderinc-bucket.s3.amazonaws.com/' + Val.imagePath + '" title="' + Val.name + '" data-node="userObject_' + Val.name + '"/>';
...
function Upload_UserDefinedObject(ID) {
    ...
    $.ajax({
        ...
        success: function (message) {
            ...
            jQuery('#txtUserDefinedObjectName,#txtUserDefinedObjectDefaultValue,#txtUserDefinedObjectScript').val('');
        },
        error: function (x, e) {
            alert("Error!");
        },
...
function getSubProjectDetails(subProjectID) {
    $.ajax({
        ...
        dataType: "json",
        success: function (response) {
            if (response[0].workFlowStatus == 'Approved')
                jQuery('#btnSave').hide();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            jQuery(response).each(function (index, val) {
                ...
                jQuery('#tblDataSources').append('<tr class="dataSourcesRow"><td style="color:black;">' + val.name + '</td><td><i mID="' + val.id + '" class="fa fa-minus-circle removeDataSource" style="color:red; cursor:pointer;" title="Remove Data Source"></i></td></tr>')
            });
        },
        error: function (response) {
...
function showPropertyValuesByNodeID(nodeID) {
    ...
    if ($selectedNodeForContext.hasClass('battery'))
        componentType = 'battery';
    else if ($selectedNodeForContext.hasClass('turbine') || $selectedNodeForContext.hasClass('Solar'))
    ...
    if (customProperties != null && customProperties.length > 0)
        jQuery(customProperties).each(function (Index, Val) {
            var htmlCode = '<tr class="dynamicProperties"><td class="font-weight-bold">' + Val.propertyName + ' (' + Val.unit + ')</td>';
            htmlCode += '<td title="' + Val.description + '">';
            if (Val.hasDatasource == 'false')
                htmlCode += '<input title="' + Val.description + '" type="text" style="width:75%;display:inline-block;" class="form-control propertyItem" value="' + Val.propertyValue + '" property="' + Val.formulaTitle + '" />';
            else {
                htmlCode += '<input title="' + Val.description + '" type="text" class="form-control propertyItem" style="width:75%;display:inline-block; margin-right:5px;font-size:10px; padding:5px;" value="'
                    + Val.propertyValue + '" property="' + Val.formulaTitle + '" /> ';
    ...
    if (outParam != null && outParam.length > 0)
        jQuery(outParam).each(function (Index, Val) {
            var htmlCode = '<tr title="' + Val.description + '"class="dynamicProperties"><td class="font-weight-bold">' + Val.propertyName + ' (' + Val.unit + ')</td>';
            htmlCode += '</tr> ';
...
function UploadFile() {
    ...
    var data = new FormData();
    data.append(files[0].name, files[0]);
    createCookie('DataSourceName', jQuery('#txtDataSourceName').val().replace(/\ /g, '#@'));
    createCookie('DataSourceDescription', jQuery('#txtDataSourceDescription').val().replace(/\ /g, '#@'));
...
function getFileName() {
    if (jQuery('#txtDataSourceName').val() == '') {
        ...
        if (file) {
            jQuery('#txtDataSourceName,#txtDataSourceDescription').val(file.name.replace('.csv', ''));
        }
    }
...
function importJSON() {
    ...
    if (file) {
        ...
        alertify.error('Select your file');
}
//#region DraFlow
...
editor.on('nodeSelected', function (id) {
    ...
    //
    jQuery('.connection').find('path').css('stroke', '#4ea9ff');
    jQuery('.node_in_node-' + id).find('path').css('stroke', '#4a8b78');
    jQuery('.node_out_node-' + id).find('path').css('stroke', '#4a8b78');
});
    ...
    switch (name) {
        case 'Aggregator':
            ...
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/Aggregator.png" class="iDesign" /><br> <span class="componentTitle">Aggregator`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Aggregator', 1, 1, pos_x, pos_y, 'Aggregator', '{\"Name\":\"Aggregator' + postfix + '\"}', mCode);
            break;
        //GeoThermal Level
        case 'Desalinization':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/GeoThermal/Desalinization.png" class="iDesign" /><br> <span class="componentTitle">Desalinization`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Desalinization', 1, 1, pos_x, pos_y, 'Desalinization', '{\"Name\":\"Desalinization' + postfix + '\"}', mCode);
            break;
        case 'Aquaculture':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/GeoThermal/Aquaculture.png" class="iDesign" /><br> <span class="componentTitle">Aquaculture`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Aquaculture', 1, 1, pos_x, pos_y, 'Aquaculture', '{\"Name\":\"Aquaculture' + postfix + '\"}', mCode);
            break;
        case 'Chiller':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/GeoThermal/Chiller.png" class="iDesign" /><br> <span class="componentTitle">Absorption Chiller`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Chiller', 1, 1, pos_x, pos_y, 'Chiller', '{\"Name\":\"Absorption Chiller' + postfix + '\"}', mCode);
            break;
        case 'GASTurbine':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/GeoThermal/GASTurbine.png" class="iDesign" /><br> <span class="componentTitle">GAS Turbine`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('GASTurbine', 1, 1, pos_x, pos_y, 'GASTurbine', '{\"Name\":\"GAS Turbine' + postfix + '\"}', mCode);
            break;
        case 'GreenHouse':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/GeoThermal/GreenHouse.png" class="iDesign" /><br> <span class="componentTitle">GreenHouse`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('GreenHouse', 1, 1, pos_x, pos_y, 'GreenHouse', '{\"Name\":\"GreenHouse' + postfix + '\"}', mCode);
            break;
        case 'OrcPowerPlant':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/GeoThermal/OrcPowerPlant.png" class="iDesign" /><br> <span class="componentTitle">Orc Power Plant`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('OrcPowerPlant', 1, 1, pos_x, pos_y, 'OrcPowerPlant', '{\"Name\":\"Orc Power Plant' + postfix + '\"}', mCode);
            break;
        case 'WasteHeat':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/GeoThermal/WasteHeat.png" class="iDesign" /><br> <span class="componentTitle">Waste Heat Recovery`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('WasteHeat', 1, 1, pos_x, pos_y, 'WasteHeat', '{\"Name\":\"Waste Heat' + postfix + '\"}', mCode);
            break;
        //real Estate
        case 'Office':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/RealEstate/House.png" class="iDesign" /><br> <span class="componentTitle">Office`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Office', 1, 1, pos_x, pos_y, 'Office', '{\"Name\":\"Office' + postfix + '\"}', mCode);
            break;
        case 'Tower':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/RealEstate/Tower.png" class="iDesign" /><br> <span class="componentTitle">Tower`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Tower', 1, 1, pos_x, pos_y, 'Tower', '{\"Name\":\"Tower' + postfix + '\"}', mCode);
            break;
        //Fleet
        case 'EvTruck':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Truck/EvTruck.png" class="iDesign" /><br> <span class="componentTitle">EV Truck`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('EvTruck', 1, 1, pos_x, pos_y, 'EvTruck', '{\"Name\":\"EV Truck' + postfix + '\"}', mCode);
            break;
        case 'FuelTrack':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Truck/FuelTrack.png" class="iDesign" /><br> <span class="componentTitle">Fuel Truck`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('FuelTrack', 1, 1, pos_x, pos_y, 'FuelTrack', '{\"Name\":\"Fuel Truck' + postfix + '\"}', mCode);
            break;
        case 'H2Truck':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Truck/H2Truck.png" class="iDesign" /><br> <span class="componentTitle">H2 Truck`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('H2Truck', 1, 1, pos_x, pos_y, 'H2Truck', '{\"Name\":\"H2 Truck' + postfix + '\"}', mCode);
            break;
        //Refinery
        case 'Boiler':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/Boiler.png" class="iDesign" /><br> <span class="componentTitle">Boiler`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Boiler', 1, 1, pos_x, pos_y, 'Boiler', '{\"Name\":\"Boiler' + postfix + '\"}', mCode);
            break;
        case 'CatalyticCracker':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/CatalyticCracker.png" class="iDesign" /><br> <span class="componentTitle">Catalytic Cracker`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('CatalyticCracker', 1, 1, pos_x, pos_y, 'CatalyticCracker', '{\"Name\":\"Catalytic Cracker' + postfix + '\"}', mCode);
            break;
        case 'CHPGeneration':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/CHPGeneration.png" class="iDesign" /><br> <span class="componentTitle">CHP Generation`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('CHPGeneration', 1, 1, pos_x, pos_y, 'CHPGeneration', '{\"Name\":\"CHP Generation' + postfix + '\"}', mCode);
            break;
        case 'Compressor':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/Compressor.png" class="iDesign" /><br> <span class="componentTitle">Compressor`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Compressor', 1, 1, pos_x, pos_y, 'Compressor', '{\"Name\":\"Compressor' + postfix + '\"}', mCode);
            break;
        case 'Cooling':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/Cooling.png" class="iDesign" /><br> <span class="componentTitle">Cooling`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Cooling', 1, 1, pos_x, pos_y, 'Cooling', '{\"Name\":\"Compressor' + postfix + '\"}', mCode);
            break;
        case 'DistillationFurnace':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/DistillationFurnace.png" class="iDesign" /><br> <span class="componentTitle">Distillation Furnace`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('DistillationFurnace', 1, 1, pos_x, pos_y, 'DistillationFurnace', '{\"Name\":\"Distillation Furnace' + postfix + '\"}', mCode);
            break;
        case 'Fan':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/Fan.png" class="iDesign" /><br> <span class="componentTitle">Fan`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Fan', 1, 1, pos_x, pos_y, 'Fan', '{\"Name\":\"Fan' + postfix + '\"}', mCode);
            break;
        case 'HydroTreater':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/HydroTreater.png" class="iDesign" /><br> <span class="componentTitle">HydroTreater`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('HydroTreater', 1, 1, pos_x, pos_y, 'HydroTreater', '{\"Name\":\"HydroTreater' + postfix + '\"}', mCode);
            break;
        case 'Isomerization':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/Isomerization.png" class="iDesign" /><br> <span class="componentTitle">Isomerization`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Isomerization', 1, 1, pos_x, pos_y, 'Isomerization', '{\"Name\":\"Isomerization' + postfix + '\"}', mCode);
            break;
        case 'MeroxTreater':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/MeroxTreater.png" class="iDesign" /><br> <span class="componentTitle">MeroxTreater`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('MeroxTreater', 1, 1, pos_x, pos_y, 'MeroxTreater', '{\"Name\":\"MeroxTreater' + postfix + '\"}', mCode);
            break;
        case 'Pump':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/Pump.png" class="iDesign" /><br> <span class="componentTitle">Pump`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Pump', 1, 1, pos_x, pos_y, 'Pump', '{\"Name\":\"Pump' + postfix + '\"}', mCode);
            break;
        case 'Machine':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/machine.png" class="iDesign" /><br> <span class="componentTitle">Machine`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Machine', 1, 1, pos_x, pos_y, 'Machine', '{\"Name\":\"Machine' + postfix + '\"}', mCode);
            break;
        case 'Process':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Refinery/process.png" class="iDesign" /><br> <span class="componentTitle">Process`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Process', 1, 1, pos_x, pos_y, 'Process', '{\"Name\":\"Process' + postfix + '\"}', mCode);
            break;
        //
        case 'Fleet':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/Level1/fleet.png" class="iDesign" /><br> <span class="componentTitle">Fleet`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Fleet', 0, 1, pos_x, pos_y, 'Fleet', '{\"Name\":\"Fleet' + postfix + '\"}', mCode);
            break;
        case 'Operation':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/Level1/operation.png" class="iDesign" /><br> <span class="componentTitle">Operation`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Operation', 0, 1, pos_x, pos_y, 'Operation', '{\"Name\":\"Operation' + postfix + '\"}', mCode);
            break;
        case 'RealState':
            var mCode = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/Level1/RealState.png" class="iDesign" /><br> <span class="componentTitle">RealState`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('RealState', 0, 1, pos_x, pos_y, 'RealState', '{\"Name\":\"Real Estate' + postfix + '\"}', mCode);
            break;
        ...
        case 'AmmoniaPlant':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/AmmoniaPlant.png" class="iDesign" /><br> <span class="componentTitle">Ammonia Plant`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('AmmoniaPlant', 1, 1, pos_x, pos_y, 'AmmoniaPlant', '{\"Name\":\"Ammonia Plant' + postfix + '\"}', battery);
            break;
        case 'AssetReplacement':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/AssetReplacement.png" class="iDesign" /><br> <span class="componentTitle">Asset Replacement`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('AssetReplacement', 1, 1, pos_x, pos_y, 'AssetReplacement', '{\"Name\":\"Asset Replacement' + postfix + '\"}', battery);
            break;
        case 'GreenH2':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/GreenH2.png" class="iDesign" /><br> <span class="componentTitle">Green H2`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('GreenH2', 1, 1, pos_x, pos_y, 'GreenH2', '{\"Name\":\"GreenH2' + postfix + '\"}', battery);
            break;
        case 'GeoThermal':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/GeoThermal.png" class="iDesign" /><br> <span class="componentTitle">GeoThermal`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('GeoThermal', 1, 1, pos_x, pos_y, 'GeoThermal', '{\"Name\":\"GeoThermal' + postfix + '\"}', battery);
            break;
        case 'Refinery':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/Refinery.png" class="iDesign" /><br> <span class="componentTitle">Refinery`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Refinery', 1, 1, pos_x, pos_y, 'Refinery', '{\"Name\":\"Refinery' + postfix + '\"}', battery);
            break;
        case 'UpStream':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/UpStream.png" class="iDesign" /><br> <span class="componentTitle">UpStream`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('UpStream', 1, 1, pos_x, pos_y, 'UpStream', '{\"Name\":\"UpStream' + postfix + '\"}', battery);
            break;
        case 'FootPrint':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/footprint.png" class="iDesign" /><br> <span class="componentTitle">Carbon Footprint`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Footprint', 0, 1, pos_x, pos_y, 'Footprint', '{\"Name\":\"Carbon Footprint' + postfix + '\"}', battery);
            break;
        case 'Retrofit':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/retrofit.png" class="iDesign" /><br> <span class="componentTitle">Retrofit`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Retrofit', 0, 1, pos_x, pos_y, 'Retrofit', '{\"Name\":\"Retrofit' + postfix + '\"}', battery);
            break;
        case 'CrProject':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/CrProject.png" class="iDesign" /><br> <span class="componentTitle">CR Project`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('CrProject', 0, 1, pos_x, pos_y, 'CrProject', '{\"Name\":\"CR Project' + postfix + '\"}', battery);
            break;

        case 'Balancer':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/icons/loadBalancer.png" class="iDesign" /><br> <span class="componentTitle">Balancer`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Balancer', 5, 1, pos_x, pos_y, 'Balancer', '{\"Name\":\"Balancer' + postfix + '\"}', battery);
            break;
        case 'EmissionData':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/Emission.png" class="iDesign" /><br> <span class="componentTitle">Production Well Emission Data`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('ProductionWellEmission', 0, 1, pos_x, pos_y, 'ProductionWellEmission', '{\"Name\":\"ProductionVendorEmission' + postfix + '\"}', battery);
            break;
        case 'EmissionSaving':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/saving.png" class="iDesign" /><br> <span class="componentTitle">Emission Saving`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('EmissionSaving', 1, 0, pos_x, pos_y, 'EmissionSaving', '{\"Name\":\"EmissionSaving' + postfix + '\"}', battery);
            break;
        case 'TotalOut':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/TotalOutput.png" class="iDesign" /><br> <span class="componentTitle">Total Emission Out`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('TotalOut', 1, 0, pos_x, pos_y, 'TotalOut', '{\"Name\":\"TotalOut' + postfix + '\"}', battery);
            break;
        case 'Budget':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/Budget.png" class="iDesign" /><br> <span class="componentTitle">Budget`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Budget', 1, 1, pos_x, pos_y, 'Budget', '{\"Name\":\"Budget' + postfix + '\"}', battery);
            break;
        case 'Capex':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/Level1/Capex.png" class="iDesign" /><br> <span class="componentTitle">Capex/Opex`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('Capex', 1, 1, pos_x, pos_y, 'Capex', '{\"Name\":\"Capex/Opex' + postfix + '\"}', battery);
            break;
        case 'EmissionReductionTarget':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/Level1/EmissionReductionTarget.png" class="iDesign" /><br> <span class="componentTitle">Emission Reduction Target`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('EmissionReductionTarget', 1, 1, pos_x, pos_y, 'EmissionReductionTarget', '{\"Name\":\"EmissionReductionTarget' + postfix + '\"}', battery);
            break;
        case 'VendorEmission':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/Carbon/Vendor.png" class="iDesign" /><br> <span class="componentTitle">Vendor Emission Data`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('VendorEmission', 0, 1, pos_x, pos_y, 'VendorEmission', '{\"Name\":\"VendorEmissionData' + postfix + '\"}', battery);
            break;
        ...
        case 'battery':
            var battery = `
            <div class="innerDiv" priority="0">
              <div class="title-box"><img draggable="false" src="../Images/icons/Battery.png" class="iDesign" /><br> <span class="componentTitle">Battery`+ postfix + `</span></div>
            </div>
            `;
            editor.addNode('battery', 1, 1, pos_x, pos_y, 'battery', '{\"Name\":\"battery' + postfix + '\"}', battery);
            break;
        case 'turbine':
            var slackchat = `
             <div class="innerDiv" priority="2">
              <div class="title-box"><img draggable="false" src="../Images/icons/Turbine.png" class="iDesign" /><br> <span class="componentTitle">Turbines`+ postfix + `</span></div>
            </div>
              `;
            editor.addNode('turbine', 0, 1, pos_x, pos_y, 'turbine', '{\"Name\":\"turbines' + postfix + '\"}', slackchat);
            break;
        case 'Solar':
            var slackchat = `
            <div class="innerDiv" priority="0">
               <div class="title-box"><img draggable="false" src="../Images/icons/solar.png" class="iDesign" /><br> <span class="componentTitle">Solar Panel`+ postfix + `</span> </div>
            </div>
              `
            editor.addNode('Solar', 0, 1, pos_x, pos_y, 'Solar', '{\"Name\":\"Solar Panel' + postfix + '\"}', slackchat);
            break;
        case 'Grid':
            var slackchat = `
           <div class="innerDiv" priority="3">
              <div class="title-box"><img draggable="false" src="../Images/icons/Grid.png" class="iDesign" /><br> <span class="componentTitle">Grid`+ postfix + `</span> </div>
            </div>
              `
            editor.addNode('Grid', 1, 1, pos_x, pos_y, 'Grid', '{\"Name\":\"Grid' + postfix + '\"}', slackchat);
            break;
        case 'cloud':
            var slackchat = `
            <div class="innerDiv" priority="5">
              <div class="title-box"><img draggable="false" src="../Images/icons/cloud.png" class="iDesign" /><br> <span class="componentTitle">Cloud`+ postfix + `</span></div>
            </div>
              `
            editor.addNode('cloud', 1, 0, pos_x, pos_y, 'cloud', '{\"Name\":\"cloud' + postfix + '\"}', slackchat);
            break;
        case 'WaterStorage':
            var slackchat = `
            <div class="innerDiv" priority="5">
              <div class="title-box"><img draggable="false" src="../Images/icons/WaterStorage.png" class="iDesign" /><br> <span class="componentTitle">Water Storage`+ postfix + `</span></div>
            </div>
              `
            editor.addNode('WaterStorage', 1, 1, pos_x, pos_y, 'WaterStorage', '{\"Name\":\"Water Storage' + postfix + '\"}', slackchat);
            break;
        case 'GreenH2Storage':
            var slackchat = `
            <div class="innerDiv" priority="7">
              <div class="title-box"><img draggable="false" src="../Images/icons/StorageH2.png" class="iDesign" /><br> <span class="componentTitle">Green H2 Storage`+ postfix + `</span></div>
            </div>
              `
            editor.addNode('GreenH2Storage', 1, 1, pos_x, pos_y, 'GreenH2Storage', '{\"Name\":\"Green H2 Storage' + postfix + '\"}', slackchat);
            break;
        case 'ExternalGraySupplier':
            var slackchat = `
            <div class="innerDiv" priority="7">
              <div class="title-box"><img draggable="false" src="../Images/icons/GrayStorageH2.png" class="iDesign" /><br> <span class="componentTitle">External Gray Supplier`+ postfix + `</span></div>
            </div>
              `
            editor.addNode('ExternalGraySupplier', 0, 1, pos_x, pos_y, 'ExternalGraySupplier', '{\"Name\":\"External Gray Supplier' + postfix + '\"}', slackchat);
            break;
        case 'Mobility':
            var slackchat = `
            <div class="innerDiv" priority="8">
              <div class="title-box"><img draggable="false" src="../Images/icons/Mobility.png" class="iDesign" /><br> <span class="componentTitle">Mobility`+ postfix + `</span></div>
            </div>
              `
            editor.addNode('Mobility', 1, 0, pos_x, pos_y, 'Mobility', '{\"Name\":\"Mobility' + postfix + '\"}', slackchat);
            break;
        case 'industriese':
            var slackchat = `
            <div class="innerDiv" priority="8">
              <div class="title-box"><img draggable="false" src="../Images/icons/industriese.png" class="iDesign" /><br> <span class="componentTitle">Industries`+ postfix + `</span></div>
            </div>
              `
            editor.addNode('industriese', 1, 0, pos_x, pos_y, 'industriese', '{\"Name\":\"industries' + postfix + '\"}', slackchat);
            break;
        case 'electrolyzer':
            var slackchat = `
            <div class="innerDiv" priority="4">
              <div class="title-box"><img draggable="false" src="../Images/icons/electrolyzer.png" class="iDesign" /><br> <span class="componentTitle">Electrolyser`+ postfix + `</span></div>
            </div>
              `
            editor.addNode('electrolyzer', 2, 2, pos_x, pos_y, 'electrolyzer', '{\"Name\":\"electrolyser' + postfix + '\"}', slackchat);
            break;
        case 'electricityLB':
            var slackchat = `
            <div class="innerDiv" priority="1">
              <div class="title-box"><img draggable="false" src="../Images/icons/loadBalancerElectricity.png" class="iDesign" /><br> <span class="componentTitle">Electricity LB`+ postfix + `</span></div>
            </div>
              `
            editor.addNode('electricityLB', 4, 3, pos_x, pos_y, 'electricityLB', '{\"Name\":\"electricity LB' + postfix + '\"}', slackchat);
            break;
        case 'hydrogenLB':
            var slackchat = `
            <div class="innerDiv" priority="6">
              <div class="title-box"><img draggable="false" src="../Images/icons/loadBalancer.png" class="iDesign" /><br> <span class="componentTitle">Hydrogen LB`+ postfix + `</span></div>
            </div>
              `
            editor.addNode('hydrogenLB', 3, 3, pos_x, pos_y, 'hydrogenLB', '{\"Name\":\"hydrogen LB' + postfix + '\"}', slackchat);
            break;
        case 'Optimization':
            var slackchat = `
            <div class="innerDiv" priority="6">
              <div class="title-box"><img draggable="false" src="../Images/icons/optimization.png" class="iDesign" /><br> <span class="componentTitle">Optimization`+ postfix + `</span></div>
            </div>
              `
            editor.addNode('Optimization', 3, 3, pos_x, pos_y, 'Optimization', '{\"Name\":\"Optimization' + postfix + '\"}', slackchat);
            break;
        case 'Prob':
            var slackchat = `
            <div class="innerDiv" priority="6">
              <div class="title-box">`+
                ...
                + `<br> <span class="componentTitle">Prob` + postfix + `</span></div>
            </div>
              `
            editor.addNode('Optimization', 1, 0, pos_x, pos_y, 'Optimization', '{\"Name\":\"Prob' + postfix + '\"}', slackchat);
            break;
        default:
            if (name.indexOf('userObject_') != -1) {
                postfix = createPostiFixForComponent(name.replace('userObject_', ''));
                var $element = jQuery('.userDefinedObjectIcons[customName="' + name + '"]');
                var htmlCode = '<div class="innerDiv" priority="100"><div class="title-box">';
                htmlCode += '<div class="title-box"><img draggable="false" src="' + $element.find('img').attr('src') + '" class="iDesign" /><br> <span class="componentTitle">' + name.replace('userObject_', '') + postfix + '</span></div>'
                    + '</div>';
                //
                ...
                //
                editor.addNode(name.replace('userObject_', ''), coInfo[0].inputCount, coInfo[0].outputCount, pos_x, pos_y, name.replace('userObject_', ''),
                    '{\"Name\":\"' + name.replace('userObject_', '') + postfix + '\"}', htmlCode);
            }
            else if (name.indexOf('Macro_') != -1) {
                var mID = jQuery('.userMacroIcons[customName="' + name + '"]').attr('mID');
                ...
                jQuery(mMacro[0].components).each(function (Index, Val) {
                    ...
                    //
                    editor.addNode(Val.nodeName, Val.inputConnectionsCount, Val.outputConnectionsCount, Val.positionTop, Val.positionLeft, Val.nodeName,
                        JSON.stringify(mProperties), Val.nodeHtmlContent, false, Val.nodeID);
                });
                //
                //Create Connections
                jQuery(mMacro[0].components).each(function (Index, Val) {
                    if (Val.inputConnections.length > 0) {
                        jQuery(Val.inputConnections).each(function (conIndex, conVal) {
                            editor.addConnection(conVal.node, Val.nodeID, conVal.source, conVal.destination);
                        })
                    }
                    //if (Val.inputConnections.length > 0) {
